...
*   🌗 **Tema Claro/Oscuro Responsive**: Interfaz adaptativa pulida con paleta cromática de alta fidelidad e institucional (tonos guinda, oro y verde esmeralda) que cumple con estándares visuales premium y adaptabilidad móvil absoluta.
...
Si prefieres ejecutar los servicios en tu máquina local para depuración directa:

#### 1. Preparar el Backend
```bash
cd backend
npm install
# Generar los clientes Prisma de forma manual
...
#### 3. Arrancar el Procesador de Excel
```bash
cd ../excel-processor
...
Con el fin de cumplir con estrictas regulaciones gubernamentales de transparencia y seguridad de datos, Nexus implementa un sistema de logs en MongoDB mediante un **Request Interceptor** automatizado en el Backend. Cada solicitud que llega a la API es analizada y, si corresponde a una acción crítica de datos, se genera un registro estructurado inmutable:

1.  **Auditoría de Acceso**: Rastrea inicios de sesión exitosos y fallidos, dirección IP de origen, navegador del usuario y marca de tiempo exacta (`eventoInicioSesion`).
2.  **Auditoría de Operaciones**: Guarda un registro completo de qué reportes se cargaron, quién los procesó y cuántas filas se vieron afectadas (`eventoActividadDatos`).
3.  **Auditoría de Descargas**: Registra exactamente qué analista o coordinador generó y exportó reportes operativos de servicios (`logReportesGenerados`).
...
sequenceDiagram
    autonumber
    actor U as Usuario (Capturador / Coordinador)
    participant FE as Frontend (React SPA)
    participant BE as Backend (Express API)
    participant EP as Excel Processor (Python Worker)
    participant SQL as SQL Server (Prisma)
    participant MON as MongoDB (Auditoría)

    U->>FE: Sube archivo Excel (.xlsx)
    FE->>BE: POST /api/sql/transaccionesExcel/upload
    BE->>BE: Intercepta Request y Valida Permisos
    BE->>EP: POST /upload-excel (Envía archivo binario)
    Note over EP: Pandas valida estructura de columnas,<br/>tipos de datos e integridad
    alt Estructura Inválida
        EP-->>BE: Retorna error 422 (Detalle del error)
        BE-->>FE: Muestra alerta en interfaz "Archivo incompatible"
    else Estructura Válida
        EP->>SQL: Inserta filas procesadas por bloques
        SQL-->>EP: Confirma inserción exitosa
        EP-->>BE: Retorna estatus 200 "Procesado Exitosamente"
        BE->>MON: Loguea Transacción en MongoDB (Auditoría de Ingesta)
        BE-->>FE: Envía confirmación con conteo de registros insertados
        FE-->>U: Muestra notificación de éxito "Carga Completada"
    end
```

---

## 🎨 Interfaz y Guías de Estilo

El panel adopta una temática elegante con bordes suaves, efectos de desenfoque por cristal (glassmorphism) y transiciones fluidas de 200ms en todos sus botones y tarjetas interactivas.

### Paleta de Colores Corporativa
*   **Vino/Guinda Principal**: `#6B1D3E` (Claro) / `#4A1229` (Oscuro) - Identidad institucional.
*   **Oro de Contraste**: `#C09447` / `#D4A574` - Bordes, títulos destacados e indicadores visuales de primer nivel.
*   **Verde Esmeralda**: `#0D5036` - Botones de éxito y avatares de usuario.
*   **Fondo Oscuro Premium**: `#111827` / `#1F2937` - Cumple con la accesibilidad visual reduciendo el cansancio ocular.
