...
def parse_excel_robust(file_path, filename):
    xls = pd.ExcelFile(file_path)
    result_reports = []
    
    year, month = extract_year_month(filename)
    print(f"Extracted Month/Year: {month}/{year}")
    ...
    for sheet_name in xls.sheet_names:
        ...
        if header_idx is not None:
            ...
            # Print mapped columns for first sheet as debug
            if sheet_name == '03':
                print("Mapped Column Indices:")
                print(f"  Cuadrilla: {col_cuadrilla}")
                print(f"  Ubicación: {col_ubicacion}")
                print(f"  Solicitó: {col_solicito}")
                print(f"  Metros Lineales: {col_m_lineales}")
                print(f"  Metros Cuadrados: {col_m_cuadrados}")
                print(f"  Metros Cúbicos: {col_m_cubicos}")
                print(f"  Peso (Kg): {col_peso}")
            ...
            for idx in range(data_start_idx, len(df_raw)):
                ...
                # If cuadrilla contains .0 (e.g. 12.0), remove it
                ...
                folio = clean_str(row[col_folio]) if col_folio is not None else None
                ventanilla = clean_str(row[col_ventanilla]) if col_ventanilla is not None else None
...
path_to_excel = r'C:\Users\gael1\Downloads\2-. Reportes Febrero 2026.xlsx'
reports = parse_excel_robust(path_to_excel, "2-. Reportes Febrero 2026.xlsx")
